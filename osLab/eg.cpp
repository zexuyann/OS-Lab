...
void ExecComd(int k)        //执行命令
{
    ...
    char CmdTab[][COMMAND_LEN] = {"create", "open", "write", "read", "close",
                                  ...
                                  "undel", "Prompt", "udtab", "fc", "replace", "move"};
...
int FcComd(int k) {
    //todo
    short size1, size2, size, j=0;
    short s0, s1, s2, cur1, cur2;
    int flag, count = 0;
    char attrib = '\0', *FileName;
    char Buffer1[SIZE * 10];
    char Buffer2[SIZE * 10];
    FCB *fcbp;
    ...
    s0 = ProcessPath(comd[1], FileName, 2, 2, '\020');//'\20'说明找的是文件所在目录的首块号'
    s1 = FindFCB(FileName, s0, attrib, fcbp);//attrib=0，说明找的是该文件。
    if (s1 < 1) {
        cout << "\n文件不存在。\n";
        return s1;        //失败，返回
    }
    size1 = fcbp->Fsize;
    //s0 = FindPath(comd[2], attrib, 0, fcbp);
    s0 = ProcessPath(comd[2], FileName, 2, 2, '\020');
    s2 = FindFCB(FileName, s0, attrib, fcbp);
    if (s2 < 1) {
        cout << "\n文件不存在。\n";
        return s2;        //失败，返回
    }
    ...
    while (count < size) {
        for (int i = 0; i < SIZE; i++, j++) {
            Buffer1[j] = Disk[cur1][i];
            Buffer2[j] = Disk[cur2][i];
            if (Buffer1[j] != Buffer2[j]) {
                cout << "第一个不同的位置是: " << j << endl;
                cout << "第一个文件该位置的内容为:" << Buffer1[j] << "\n"
                     << "第二个文件该位置的内容为:" << Buffer2[j]<< "\n";
                return 1;
            }
            if (count == size) {
                j++;
...
int ReplaceComd(int k) {
    //todo
    short s0, s1,s11, s2,s3,s4, i, jj = 0, j = 0;//s11记录文件1的首块号
    short b,b0,bnum;
    int size;
    int count = 0;
    char attrib = '\0', *FileName,*FileName1;
    char *Buffer;
    ...
    s0 = ProcessPath(comd[1], FileName, k, 0, '\020');//取FileName所在目录的首块号
    if (s0 < 1)            //路径错误
        return s0;        //失败，返回
    ...
    if (k == 1) {//comd[2]不存在
        s2 = FindFCB(FileName, curpath.fblock, attrib, fcbp);
        if(s2==s11){
            cout<<"文件名指定的文件和被取代的文件不应该是同一个文件\n";
            return 0;
        }
        if(fcbp->Fattrib&'\01'){
            cout<<"被取代的是只读属性的文件,是否确认取代？ y/n \n";
            char in;
            cin>>in;
            if(in=='n')return 0;
        }else if(fcbp->Fattrib&'\02'||fcbp->Fattrib&'\16'){
            cout<<"具有隐藏和系统属性的文件不能被取代\n";
            return 0;
        }
    } else {////comd[2]存在

        s0 = ProcessPath(comd[2], FileName, k, 0, '\020');//取FileName所在目录的首块号
        if (s0 < 1) {
            cout << "//路径错误";
            return s0;        //失败，返回
        }
        s3 = FindFCB(FileName, s0, '\020', fcbp);        //取FileName的首块号(查其存在性)
        s2 = FindFCB(FileName1, s3, attrib, fcbp);
        if (fcbp->Fattrib & '\01') {
            cout << "被取代的是只读属性的文件,是否确认取代？ y/n \n";
            char in;
            cin >> in;
        ...
        *fcbp2 = *fcbp1;
        ...
        fcbp1->Addr = fcbp1->Fsize = 0;
        strcpy(fcbp2->FileName, FileName1);
        //cout<<"File1  "<<fcbp2->FileName<<endl;
        b0 = 0;
        while (s11 > 0) {
            b = getblock();
            if (b0 == 0)
...
int MoveComd(int k) {
    //todo
    short s01, s02, s1, s2, s3, i,s;
    short b, b0, bnum;
    int size;
    char attrib = '\040', *FileName1, *FileName2;
    string in;
    char gFileName[PATH_LEN];    //存放文件全路径名
    FCB *fcbp, *fcbp1, *fcbp2;
    int flag = 0;
    ...
    s2 = FindFCB(FileName2, s02, '\020', fcbp);//改名
    if (s2 < 0) {
//        cout << "\n文件不存在。" << endl;
//        return -1;
        if (IsName(FileName2)) {
            s = FindFCB(FileName2, s02, attrib, fcbp);    //查新文件名是否重名
            if (s < 0)            //不重名
            {
                //cout<<FileName2<<endl;
    ...
    s3 = FindFCB(FileName1, s2, attrib, fcbp);
    if (s3 > 0) {
        if(fcbp->Fattrib&&'\020'){
            cout<<"目录中已经存在同名子目录，无法覆盖。\n";
            return -6;
        }
        cout << "指定目录中存在同名文件，是否覆盖？（y/n）" << endl;
        cin >> in;
        if (in == "n") {
            cout << "命令取消" << endl;
            return 1;
...
void HelpComd()                //help命令，帮助信息(显示各命令格式)
{
    ...
    cout << "3  write <文件名> [<位置/app>[ insert]]     ——在指定位置写文件(有插入功能)。\n";
    ...
    cout << "25 fc                                     ——比较两个文件的内容。\n";
    cout << "26 replace                                ——实现文件替代。\n";
    cout << "27 move                                   ——文件“搬家”以及子目录改名。\n";
    cout << "\n 请输入命令序号以详细查看命令" << endl;
    int sw;
    cin >> sw;
    switch (sw) {
        case 1:
            cout << "创建文件：create <文件名> [<文件属性>]，创建一个指定名字的新文件，\n"
        ...
        case 2:
            ...
            break;
        case 3:
            cout << "写文件：write <文件名> [<位置>[ insert]]，命令中若无\"位置\"参数，则在写指\n"
            ...
            break;
        case 4:
            cout << "读文件：read <文件名> [<位置m> [<字节数n>]，从已打开的文件读文件内容并显示。若无\n"
            ...
            break;
        case 5:
            cout << "close <文件名>，若指定文件已打开，则关闭之，即从UOF中删除该文件\n"
                    "对应的表项。若文件未打开或文件不存在，分别给出有关信息。\n";
            break;
        case 6:
            cout << "删除文件：del <文件名>，删除指定的文件，即清除其目录项和回收\n"
        ...
        case 7:
            ...
            break;
        case 8:
            cout << "当前目录（工作目录）转移到指定目录下。指定目录不存在时，给出错误信息。\n"
                    "若命令中无目录名，则显示当前目录路径。\n";
            break;
        case 9:
            cout << "命令形式：md <目录名>\n"
            ...
            break;
        case 10:
            cout << "若指定目录为空，则删除之，否则，给出\"非空目录不能删除\"的提示。\n"
                    "不能删除当前目录。\n";
        case 11:
            cout << "命令形式：ren <原文件名> <新文件名>\n"
                    "若原文件不存在，给出错误信息。\n"
                    "若原文件存在，但正在使用，也不能改名，同样显示出错信息。\n"
                    "应检查新文件名是否符合命名规则。\n";
            break;
        case 12:
            cout << "显示修改文件属性：attrib <文件名> [±<属性>]。若命令中无\"文件属性\"参数，\n"
        ...
        case 13:
            ...
            break;
        case 14:
            cout << "显示文件内容：type <文件名>，显示指定文件的内容。\n"
                    "若指定文件不存在，则给出错误信息。\n";
            break;
        case 15:
            cout << "命令形式：rewind <文件名>\n"
                    "对指定文件操作，同时该文件变为当前操作文件\n";
            break;
        case 16:
            cout << "命令形式：fseek <文件名> <n>\n"
                    "功能：将读、写指针移到指定位置n处\n";
            break;
        case 17:
            cout << "block命令处理函数(显示文件或目录占用的盘块号)\n";
            break;
        case 18:
            cout << "closeall命令，关闭当前用户的所有文件\n";
            break;
        case 19:
            cout << "显示用户已打开文件表UOF的内容\n";
            break;
        case 20:
            cout << "命令形式：undel [<目录名>]\n"
                    "命令功能：恢复指定目录中被删除的文件\n"
...
int GetAttribMd(char *str, char &attrib) {
    //todo
    int i, len;
    char ar = '\01', ah = '\02', as = '\04';
...
int DirComd(int k)    //dir命令，显示指定目录的内容（文件名或目录名等）
{
    ...
    cout << " free" << endl;
    cout << curpath.fblock << "   " << curpath.cpath << endl;
    return 1;
}
...
int TypeComd(int k)        //type命令处理函数(显示文件内容)
{
    ...
    if (k == 1) {
        ...
        char temp[PATH_LEN];
        memcpy(temp, temppath, PATH_LEN);
        s = ProcessPath(temp, FileName, k, 0, '\020');//取FileName所在目录的首块号
        if (s < 1)            //路径错误
            return s;        //失败，返回
        s = FindFCB(FileName, s, attrib, fcbp);
    }
    strcpy(gFileName, temppath);
    i = strlen(temppath);
    if (temppath[i - 1] != '/')
...
//////
    ...
    if(k==0){//无文件名参数
        ...
        if (uof[ii_uof].attr & '\01' && uof[ii_uof].state != 1) {    //只读文件不是创建状态不能写
            cout << "\n" << temppath << "是只读文件，不能写。\n";
            return -3;
        }
        pos = uof[ii_uof].writep;    //从写指针所指位置开始写(write <文件名>)
    }else{
        if(isalnum(comd[1][0])- isalpha(comd[1][0])!=0||strstr(comd[1],"ins")!=NULL|| strstr(comd[1],"app")!=NULL){//不包含文件名
            //不包含文件名
            if (_strnicmp(comd[1], "app", 3) == 0)
                pos = uof[ii_uof].fsize + 1;    //文件尾部添加模式(write <文件名> append)
            else if (_strnicmp(comd[1], "ins", 3) == 0) {
                pos = uof[ii_uof].writep;    //从当前写指针位置开始写
                ins = 1;                    //插入模式(write <文件名> insert)
            } else {
                pos = atoi(comd[1]);        //从命令中指定位置写(write <文件名> <n>)
                if (pos <= 0) {
                    cout << "\n命令中提供的写入位置错误。\n";
                    return -4;
                }
                if (k == 2) {
                    if (_strnicmp(comd[2], "ins", 3) == 0)
                        ins = 1;            //插入模式(write <文件名> <n> insert)
                    else {
                        cout << "\n命令参数" << comd[1] << "," << comd[2] << "错误\n";
                        return -5;
                    }
                }
            }
...
int DelComd(int k)            //del(删除文件)命令处理函数
{
    ...
    s0 = ProcessPath(comd[1], FileName, k, 1, '\20');//取FileName所在目录的首块号//取FileName所在目录的首块号
    if (s0 < 1)            //路径错误
        return s0;        //失败，返回
    s = FindFCB(FileName, s0, attrib, fcbp);        //取FileName的首块号(查其存在性)
    if (s < 0) {
...
{   //todo
    ...
    readc = uof[i_uof].fsize - pos + 1;
        ...
        if(isalnum(comd[1][0])- isalpha(comd[1][0])!=0){//不包含文件名
                //cout<<111<<endl;
                pos = atoi(comd[1]);        //从命令中指定位置写
...
int CopyComd(int k)        //copy命令的处理函数：复制文件
{
    ...
    {
        if (!(FileName2[0] == '.' && FileName2[1] == '.' && strlen(FileName2) == 2)) {
            if (!(comd[2][0]=='/'&&strstr(comd[2],"/"))) {
                cout << "\n命令中的目标文件名错误。\n";
                return -2;
            }
        }
    ...
    FileName2 = FileName1;///////////////////
    b = FindFCB(FileName1, s02, attrib, fcbp);//需查目录中有没有文件FileName1
    if (b >= 0) {
        cout << "\n有同名文件，不能复制。\n";
        return -4;
    }
...
int ParseCommand(char *p)    //将输入的命令行分解成命令和参数等
{
    ...
    for (i = 0; i < CK; i++)                    //初始化comd[][]
        comd[i][0] = '\0';
    for (k = 0; k < CK; k++) {    //分解命令及其参数，comd[0]中是命令，comd[1],comd[2]...是参数
        for (i = 0; *p != '\0'; i++, p++)
            if (*p != ' ')                //空格是命令、参数之间的分隔符
                comd[k][i] = *p;        //取命令标识符
            else {
